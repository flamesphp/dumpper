...
/**
 * Detects CSS color values (named, hex, rgb/rgba, hsl/hsla) and shows a color swatch
 * together with all equivalent color representations.
 *
 * @internal
 */
class DumpParsersColor implements DumpParserInterface
{
    ...
    public function parse(mixed &$variable, mixed $varData): mixed
    {
        ...
        $value    = <<<HTML
<div style="background:{$variable}" class="_dumpper-color-preview">{$variable}</div><strong>hex:</strong> {$variants['hex']}
    ...
    private function _convert(string $color): array
    {
        ...
        foreach ($variants as $type => &$variant) {
            ...
            switch ($type) {
                ...
                case 'rgb':
                    $rgb = $decimalColors;
                    if (isset($alpha)) { $rgb[] = $alpha; $a = 'a'; } else { $a = ''; }
                    $variant = "rgb{$a}( " . implode(', ', $rgb) . ' )';
                    break;
                case 'hsl':
                    $rgb = $this->_RGBtoHSL($decimalColors);
                    if ($rgb === null) { unset($variants[$type]); break; }
                    if (isset($alpha)) { $rgb[] = $alpha; $a = 'a'; } else { $a = ''; }
